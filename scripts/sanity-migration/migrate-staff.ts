...
import { staff as staffMembers } from "../../src/constants/staff";
...
async function migrateStaff() {
  ...
  for (const staff of staffMembers) {
    try {
      // Transform personalInfo to match schema structure
      const transformPersonalInfo = (info: any) => {
        if (!info) return undefined;

        const transformLocaleData = (localeData: any) => {
          if (!localeData) return undefined;

          return {
            bio: localeData.bio || '',
            contact: localeData.contact || { phone: '', email: '', location: '' },
            social: localeData.social || { linkedin: '', twitter: '' },
            specialties: localeData.specialties || [],
            experience: localeData.experience || [],
            education: localeData.education || [],
            memberships: localeData.memberships || [],
            hobbies: localeData.hobbies || [],
            achievements: Array.isArray(localeData.achievements)
              ...
              : [],
            academicActivities: localeData.academicActivities || [],
            publications: localeData.publications || [],
        ...
        return {
          flags: info.flags || {},
          am: transformLocaleData(info.am),
          en: transformLocaleData(info.en),
          ru: transformLocaleData(info.ru),
        };
      };
      ...
      const doc = {
        ...
        personalInfo: transformPersonalInfo(staff.personalInfo),
      };
      ...
      console.log(
        `✓ Migrated: ${staff.name.en || staff.name.am} (ID: ${staff.id})`,
      );
    } catch (error) {
